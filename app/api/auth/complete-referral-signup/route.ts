...
export async function POST(request: Request) {
  ...
  // Ensure the request is made by the new user themselves
  if (!user || user.id !== body.new_user_id) {
  ...
  try {
    ...
    const { data: existingReferral, error: existingReferralError } = await supabase
      ...
      .eq("created_by_user_id", referrer_id) // Assuming we link the new user's reward code to the referrer
      .maybeSingle();
    ...
    const { data: newUserDiscountCode, error: newUserDiscountError } = await supabase
      .from("discount_codes")
      .insert({
        code: `NEWUSER-${new_user_id.substring(0, 8)}`,
