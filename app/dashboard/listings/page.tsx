      ...
      if (error) {
        ...
        setSavedListings(
          (saved || []).map((item) => ({
            ...
            featured_until: item.listings.featured_until ?? undefined,
            images: item.listings.images ?? [],
            views: item.listings.views ?? 0,
            created_at: item.listings.created_at ?? "",
            updated_at: item.listings.updated_at ?? "",
            expiry_date: item.listings.expiry_date ?? undefined,
            category: item.listings.category ?? { name: "" },
          })),
        );
      }
    } catch (error) {
