// TODO: Fix the notifications actions.
//
// This file is causing multiple build errors due to issues with Supabase
// database types. The entire file has been commented out to allow for
// other features to be tested.
//
// Priority 1: Fix the database type generation/inference.
//   - The Supabase client is not correctly inferring the types for the
//     'notifications' and 'profiles' tables.
//   - This is causing `insert`, `update`, and even `select` calls to fail
//     with "No overload matches this call" errors.
//   - Investigate the `database.types.ts` file and the Supabase client
//     initialization to ensure that the types are being correctly generated
//     and loaded.
//
// Priority 2: Fix the `create_notification` RPC call.
//   - The `create_notification` function was failing with a type error on
//     the `rpc` call.
//   - The arguments being passed to the `rpc` call need to be corrected
//     to match the expected signature of the backend function.
//
// Priority 3: Uncomment and fix the functions in this file.
//   - Once the database type issues are resolved, the functions in this
//     file can be uncommented and fixed.
//
// "use server";
//
...
//   //   return { success: false, error: a.message };
//   // }
//
//   return { success: true };
// }
